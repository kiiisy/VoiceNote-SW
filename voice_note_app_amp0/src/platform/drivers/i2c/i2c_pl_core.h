...
public:
    ...
    bool ReadSubAddr(uint8_t subaddr, uint8_t *rx, size_t len);
...
private:
    ...
    bool WriteNoStop(const uint8_t *data, size_t len);
    bool ReadWithStop(uint8_t *rx, size_t len);

    // コールバック系
    static void SendDoneHandler(void *ref);
    static void ReceiveDoneHandler(void *ref);
    static void StatusDoneCandler(void *ref);
    void        SendDone();
    void        ReceiveDone();
    void        StatusDone();
