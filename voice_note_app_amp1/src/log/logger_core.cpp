...
extern "C" __attribute__((weak)) const char *log_cpu_tag() { return "cpu1"; }
...
void Logger::vprint(Level lvl, const char *file, int32_t line, const char *func, const char *fmt, va_list ap)
{
    if (lvl > level_) {
        return;
    }
    ...
    int32_t m = vsnprintf(buf + n, sizeof(buf) - n, fmt, ap);
    if (m < 0) {
        m = 0;
    }
    n += (m >= static_cast<int32_t>(sizeof(buf)) - n) ? static_cast<int32_t>(sizeof(buf)) - n - 1 : m;

#if LOG_USE_COLOR
    n += snprintf(buf + n, sizeof(buf) - n, "\x1b[0m");
#endif
    if (n < static_cast<int32_t>(sizeof(buf)) - 1 && buf[n - 1] != '\n') {
        buf[n++] = '\n';
        buf[n]   = '\0';
    }
...
void Logger::log(Level lvl, const char *file, int32_t line, const char *func, const char *fmt, ...)
{
    if (lvl > level_) {
        return;
    }
...
Logger::Scope::Scope(const char *file, int32_t line, const char *func, const char *name)
